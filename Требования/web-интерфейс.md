...
## Имена объектов
Имена объектов должны удовлетворять следующим ограничениям:
- *ДЗ-ИО-1*: любой существующий объект должен иметь имя
- *ДЗ-ИО-2*: имя объекта должно быть не длиннее 30 символов
- *ДЗ-ИО-3*: имя объекта не должно быть пустым
- *ДЗ-ИО-4*: имя объекта не должно начинаться или заканчиваться на пробелы, содержать больше 1 пробела подряд
- *ДЗ-ИО-5*: имя объекта может состоять только из пробелов, символов русского и английского алфавитов (в верхнем и нижнем регистре), а также следующих символов: ``~`!@#%()-_+=:;.,?``
- *ДЗ-ИО-6*: имена одновременно существующих объектов не могут совпадать

## Типы объектов
Типы объектов должны удовлетворять следующим ограничениям:
- *ДЗ-ТО-1*: любой существующий объект должен иметь ровно один из допустимых типов: `abc`, `regex`, `dfa` или `nfa`

## Значения объектов
Допустимые значения объектов определяются их типом и выглядят следующим образом.
- *ДЗ-ЗО-ABC*: значения объектов типа "алфавит" (`abc`) должны удовлетворять следующим требованиям:
  - *ДЗ-ЗО-ABC-1*: значением алфавита является непустое множество символов
  - *ДЗ-ЗО-ABC-2*: каждый символ в алфавите может встречаться не более 1 раза
  - *ДЗ-ЗО-ABC-3*: в алфавит не могут входить символы не из таблицы [ASCII](https://ru.wikipedia.org/wiki/ASCII), а также пробелы и следующие символы: `*()|`
  - *ДЗ-ЗО-ABC-4*: все символы в алфавите должны быть упорядочены по возрастанию их номера в таблице ASCII

- *ДЗ-ЗО-REGEX*: значения объектов типа "регулярное выражение" (РВ, `regex`) должны удовлетворять следующим требованиям

- *ДЗ-ЗО-NFA*: значения объектов типа "недетерминированный конечный автомат" (НКА, `NFA`) должны удовлетворять следующим требованиям

- *ДЗ-ЗО-DFA*: значения объектов типа "детерминированный конечный автомат" (ДКА, `DFA`) должны удовлетворять следующим требованиям
