...
### `create`
 `create <type> <name> [from <old name>]` - создаёт именованный объект с именем `<name>` и типом `<type>`. Если указан существующий объект `<old name>` (он может иметь отличающийся тип), то результат будет эквивалентен ему, в противном случае пользователь должен будет описать объект в формате, соответствующем его типу

### `delete`
`delete <name>...` - удаляет именованный объект (объекты)
...
### `echo`
`echo <text>` - выводит в консоль текст `<text>`. Пробелы в начале и конце текста игнорируются

### `equiv`
`equiv <name> <name>` - проверяет эквивалентность двух объектов (они могут иметь отличающиеся типы)

### `help`
`help` - выводит в консоль список поддерживаемых команд
...
### `list`
`list [<type>]` - отображает список существующих именованных объектов. Если указан аргумент `<type>`, то отображаются только объекты этого типа
...
### `print`
`print <name>...` - печатает содержимое именованного объекта (объектов) в консоль
...
### `samples`
`samples <name>... [number]` - печатает в консоль не более `number` строк, входящих в язык, задаваемый именованным объектом (объектами). Если `number` не указан, то выбирается значение по умолчанию (10)
